...
const MAX_TS_POINTS = 2000;   // cap timeseries length
const SIGNAL_HISTORY = 50;    // save last N signals
...
async function getRealizedVol30d(symbol){
  ...
  try{
    ...
    const std = Math.sqrt(rets.reduce((s,x)=>s+x*x,0)/rets.length - Math.pow(rets.reduce((s,x)=>s+x,0)/rets.length,2));
...
function buildSignal(series){
  // use vega-weighted if available, else atm, else null
  ...
  // Cross detection (use last two points)
  ...
  const window = y.slice(-252); // ~1y if running every 30min will just be capped later
  ...
  const tactics = rec==='Long Volatility'
    ? 'Buy straddles/strangles; keep delta ~0; consider long gamma.'
    : rec==='Short Volatility'
      ? 'Sell premium/iron condors; keep tight risk; avoid event risk.'
      : 'No edge: keep neutral/hedged.';

  return {
    ts: new Date().toISOString(),
    recommendation: rec,
    strength,
    size_hint: sizing,   // 0.5..1.0 notional multiplier
    reason: reason.join('; '),
    last_iv: last,
    ema20: fLast,
    ema100: sLast
...
async function buildForAsset(symbol){
  ...
  // Smile file (only meaningful when we had options/IVs)
